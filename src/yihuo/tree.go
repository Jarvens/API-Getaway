...
func (n *node) addRoute(path string, handle Handler, context Context) {
	fullPath := path
	n.priority++
	...
	walk:
		for {
			...
			if i < len(path) {
				...
				n.insertChild(numParams, path, fullPath, handle, context)
				return
...
func (n *node) insertChild(numParams uint8, path, fullPath string, handle Handler, context Context) {
...
func (n *node) getValue(path string) (handle Handler, p Params, context *Context, tsr bool) {
